...
public class Main {
	...
	private void waitForConnections() throws IOException {
		...
		while ( response != JOptionPane.YES_OPTION && response != JOptionPane.NO_OPTION) {
			response = JOptionPane.showOptionDialog(MainGUI.getInstance(), "Incoming connection detected from " + inConn.getInetAddress().getHostName() + ".\nDo you want to start a private chat or a multi-part chat?"
					, "Incoming connection", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
		}
	 ...
	 * @param conn - The socket to start a new chat with
	 */
	public void createNewChat(Socket conn) {
		ChatThread chatThread = new ChatThread(conn.getInetAddress() == null ? "default" : conn.getInetAddress().getHostAddress());
	...
	public void addToMultiChat(Socket conn) {
		if (MainGUI.getInstance().getChats().size() == 0) {
			...
			if (multiChat == null) {
				...
				String answer = null;
				while (answer != null) {
					answer = (String) JOptionPane.showInputDialog(MainGUI.getInstance(), "Choose which chat you would like to add the new connection to:", "Multi-chat", JOptionPane.PLAIN_MESSAGE, null, options, options[0]);
				}
